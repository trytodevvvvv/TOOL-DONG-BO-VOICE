...
def main():
    parser = argparse.ArgumentParser(description="Đồng bộ timeline CapCut từ file JSON Timestamps riêng biệt")
    ...
    try:
        ...
        print(f"Error: Không thể đọc hoặc phân tích cú pháp file JSON: {e}")
        sys.exit(1)

    # Support flexible JSON schema
    voice_config = timestamp_data.get("voice") or timestamp_data.get("audio")
    voice_file = None
    if isinstance(voice_config, dict):
        voice_file = voice_config.get("file") or voice_config.get("path")
    elif isinstance(voice_config, str):
        voice_file = voice_config

    if not voice_file:
        # Auto-detect audio file if missing in JSON schema
    ...
    if not voice_file:
        try:
            # Fallback to the first imported audio material
    ...
    # Read visuals from JSON
    visuals_list = timestamp_data.get("visuals") or timestamp_data.get("clips") or timestamp_data.get("elements")
    sentences = timestamp_data.get("sentences")

    # 3. LOAD PROJECT VIA pyJianYingDraft early so we can match materials
    try:
    ...
    # Sắp xếp danh sách ảnh/video đã nhập theo bảng chữ cái để đồng bộ theo thứ tự 1-đến-1
    imported_videos.sort(key=lambda m: os.path.basename(m.get("path") or "").lower())

    if not visuals_list and sentences:
        # Auto-construct visuals list from sentences in chronological order
        visuals_list = []
        for idx, sent in enumerate(sentences):
            start = float(sent.get("start") if sent.get("start") is not None else (sent.get("start_us", 0.0) / 1e6))
            end = float(sent.get("end") if sent.get("end") is not None else (sent.get("end_us", 0.0) / 1e6))
            ...
            if idx < len(imported_videos):
                ...
                print(f"Warning: Số lượng timeline trong file JSON nhiều hơn số lượng ảnh/video đã nhập (thiếu ảnh cho mốc thứ {idx+1}).")

    if not visuals_list:
        print("Error: Không tìm thấy trường 'visuals', 'clips', 'elements' hoặc 'sentences' trong file JSON.")
        sys.exit(1)
    ...
    for idx, visual in enumerate(visuals_list):
        ...
        if not v_file:
            print(f"Warning: Clip thứ {idx+1} trong JSON thiếu thông tin file.")
            warnings_count += 1
            continue
    ...
    # 7. REBUILD TRACKS
    # Giữ lại các track có sẵn của người dùng (BGM, âm thanh, text)
